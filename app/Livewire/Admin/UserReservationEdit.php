...
class UserReservationEdit extends Component
{
    ...
    public function mount($reservationId)
    {
        ...
        $this->instructors = User::instructors()->get();
        $this->instructor_id = $this->reservation->instructor_id;
    ...
    public function updateReservation()
    {
        ...
        $existingReservation = Reservation::where('instructor_id', $this->instructor_id)
        ...
        if ($existingReservation) {
            ...
            session()->flash('error', 'The selected instructor already has a reservation for the selected date and time.');
            return;
        }
        ...
        $instructor = User::find($this->instructor_id); // get the instructor
        $client = User::find($this->reservation->client_id); // get the client

        $emails = [$instructor->email, $client->email];
    ...
    {
        return view('livewire.admin.user-reservation-edit', [
